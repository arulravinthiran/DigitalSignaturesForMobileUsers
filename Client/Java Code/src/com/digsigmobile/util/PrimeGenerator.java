...
public class PrimeGenerator {
	...
	public PrimeGenerator(int minBitLength, int certainty, Random random) {
		//The bit length of the prime will exceed minBitLength
		if (minBitLength<512) throw new IllegalArgumentException("Strong/Safe primes must be " +
	...
	public BigInteger getNextSafePrime(BigInteger minimumValue) {
		BigInteger p, q;
		q = minimumValue;
		
		for (;;) {
			// p <- 2q + 1
			p = q.shiftLeft(1).add(BigIntegerMath.ONE);
			if (p.isProbablePrime(certainty) && q.isProbablePrime(certainty)) break;
...
/*	
   public BigInteger getNextSafePrime(BigInteger minimumValue) {
	   ...
	   for(;;) {
		   q = p.subtract(BigIntegerMath.ONE).shiftRight(1);
		   if (q.isProbablePrime(certainty) && p.isProbablePrime(certainty)) break;
		   p = p.nextProbablePrime();
	   }
	   return p;
   } 
/*
   public boolean isSafePrime(BigInteger prime) {
	   BigInteger p = prime.subtract(BigIntegerMath.ONE).divide(BigIntegerMath.TWO);
	   return (p.isProbablePrime(certainty) && prime.isProbablePrime(certainty));
   }
